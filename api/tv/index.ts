...
const LIST_ENDPOINTS = ['popular', 'top_rated', 'on_the_air', 'airing_today']
...
export default async function handler(req: VercelRequest, res: VercelResponse): Promise<void> {
  await withTmdbErrorHandling(res, async () => {
    ...
    if (segments.length === 1 && LIST_ENDPOINTS.includes(first)) {
      await tmdbGet(res, `/tv/${first}`, { page, language })
      return
    }
    ...
    if (segments.length === 1 && first === 'discover') {
      await tmdbGet(res, '/discover/tv', {
        ...
        first_air_date_year: queryParam(req, 'first_air_date_year'),
    ...
    if (segments.length === 1 && first === 'genres') {
      await tmdbGet(res, '/genre/tv/list', { language })
      return
    }

    if (segments.length === 2 && first === 'trending') {
      await tmdbGet(res, `/trending/tv/${encodeURIComponent(second)}`, { page, language })
      return
    }

    if (segments.length >= 1 && /^\d+$/.test(first)) {
      const seriesId = first

      if (segments.length === 1) {
        await tmdbGet(res, `/tv/${encodeURIComponent(seriesId)}`, { language })
        return
      }
      ...
      if (segments.length === 2 && DETAIL_SUBRESOURCES.includes(second)) {
        ...
        await tmdbGet(res, `/tv/${encodeURIComponent(seriesId)}/${second}`, query)
        return
      }
    }
